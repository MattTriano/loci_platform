...
NOTE: This setup assumes a specific AWS org setup where there's one admin-management account (for owning the domain name), and three project accounts (one for dev, one for staging, and one for prod).

You should set up three `.tfvars` files: `dev.tfvars`, `staging.tfvars`, and `prod.tfvars`. They'll all have the fields included in the `dev.tfvars` example below, and the `prod.tfvars` file should have two extra fields: `admin_mgmt_zone_id` and `admin_mgmt_dns_role_arn`. See the "Cross-Account DNS Role:" section below for instructions on determining those values.

```tfvars
environment              = "dev"
base_domain              = "bikeinfra.com"
extra_cors_origins       = ["http://something_resolving_to_your_dev_machines_ip_addr:a_port"]

cities = {
  chicago = {
    routing_api_key = "<random output generated by running 'openssl rand -hex 32'>"
  }
}
```
...
```bash
cd infra/
AWS_PROFILE=<env-profile> tofu init -reconfigure -var-file=<env>.tfvars
...
The env's hosted zone (e.g. `dev.bikeinfra.com`) is created early in the apply,
...
2. Go to the `<env>.bikeinfra.com` hosted zone
3. Copy the 4 NS record values
4. Open the **mgmt account's** Route53 console
5. Go to the `bikeinfra.com` hosted zone
...
- If DNS doesn't resolve immediately, it may be negative caching from earlier lookups.
  ...
  `dig A <city>.<env>.bikeinfra.com @<nameserver>`
...
- `bike_map_url` — the public URL (e.g. `https://<city>.dev.bikeinfra.com`)
...
Output: `/hostedzone/Z0123456789ABCDEFGHIJ`. The ID is the trailing part (`Z0123456789ABCDEFGHIJ`), which will be `admin_mgmt_zone_id` in your `prod.tfvars`.
