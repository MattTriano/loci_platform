...
# SSM Parameter Store
#
# Tofu writes per-(env, city) values here so Airflow can read them at
# task-run time without needing them in .env. The routing API key is
# generated by tofu and the Lambda reads it on cold start.
#
# Path layout: /loci/<env>/<city>/bike-map/<name>
#
# ignore_changes on routing_api_key.value means `tofu apply` won't
# overwrite a key that was rotated out-of-band. To force a rotation,
# taint random_password.routing_api_key.
# -----------------------------------------------------------------------------

resource "random_password" "routing_api_key" {
  length  = 64
  special = false
}

locals {
  ssm_prefix = "/${var.basename}/${var.environment}/${var.city}/bike-map"
}
...
resource "aws_ssm_parameter" "routing_graph_bucket" {
  name  = "${local.ssm_prefix}/routing-graph-bucket"
  type  = "String"
  value = aws_s3_bucket.routing_graph.bucket
}

resource "aws_ssm_parameter" "routing_lambda_arn" {
  name  = "${local.ssm_prefix}/routing-lambda-arn"
  type  = "String"
  value = aws_lambda_function.routing_api.arn
}

resource "aws_ssm_parameter" "routing_api_url" {
  name  = "${local.ssm_prefix}/routing-api-url"
  type  = "String"
  value = "https://${local.api_domain}/route"
}
