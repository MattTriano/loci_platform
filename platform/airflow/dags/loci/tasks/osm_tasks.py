...
def _get_collector(conn_id: str, task_logger: Logger) -> OsmCollector:
    pg_engine = get_postgres_engine(conn_id=conn_id, logger=task_logger)
    tracker = IngestionTracker(engine=pg_engine)
    return OsmCollector(engine=pg_engine, tracker=tracker)


@task
def run_full_update(
    spec: OsmDatasetSpec, update_config: DatasetUpdateConfig, conn_id: str, task_logger: Logger
) -> bool:
...
@task_group
def update_osm_table(
    dataset_spec: OsmDatasetSpec,
