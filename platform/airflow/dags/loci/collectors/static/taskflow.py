# /loci_platform/platform/airflow/dags/loci/collectors/dkan/taskflow.py
...
def _get_collector(conn_id: str, task_logger: Logger) -> StaticFileCollector:
    pg_engine = get_postgres_engine(conn_id=conn_id, logger=task_logger)
    tracker = IngestionTracker(engine=pg_engine)
    return StaticFileCollector(engine=pg_engine, tracker=tracker)
