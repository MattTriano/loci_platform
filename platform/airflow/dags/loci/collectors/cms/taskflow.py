# /loci_platform/platform/airflow/dags/loci/collectors/cms/taskflow.py
...
def _get_collector(conn_id: str, task_logger: Logger) -> CMSCollector:
    pg_engine = get_postgres_engine(conn_id=conn_id, logger=task_logger)
    tracker = IngestionTracker(engine=pg_engine)
    return CMSCollector(engine=pg_engine, tracker=tracker)
...
@task_group
def update_cms_table(update_config: DatasetUpdateConfig, conn_id: str, task_logger: Logger) -> None:
