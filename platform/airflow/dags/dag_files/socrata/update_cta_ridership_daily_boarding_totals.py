import datetime as dt
import logging

from airflow.sdk import dag
from loci.collectors.socrata.taskflow import update_socrata_table
from loci.sources.update_configs import (
    CTA_RIDERSHIP_DAILY_BOARDING_TOTALS_UC as UPDATE_CONFIG,
)

task_logger = logging.getLogger("airflow.task")


CONN_ID = "gis_dwh_db"


@dag(
    schedule=UPDATE_CONFIG.update_cron,
    start_date=dt.datetime(2022, 11, 1),
    catchup=False,
    tags=["socrata", "update", "chicago", "transit", "cta"],
)
def update_cta_ridership_daily_boarding_totals():
    update_socrata_table(update_config=UPDATE_CONFIG, conn_id=CONN_ID, task_logger=task_logger)


update_cta_ridership_daily_boarding_totals()
